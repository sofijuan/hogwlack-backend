...
export const validateWorkspaceSearch = [
  query('name')
    .isLength({ min: 3, max: 50 })
    .withMessage(
      'Debe proporcionar un name para de al menos 3 caracteres para buscar workspaces'
    ),
  validate
];

// Validación para busqueda de Channel
export const validateChannelSearch = [
  query('name')
    .isLength({ min: 3, max: 50 })
    .withMessage(
      'Debe proporcionar un name para de al menos 3 caracteres para buscar channels'
    ),
  validate
];
